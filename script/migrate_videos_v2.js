    ...
    return new Promise((resolve, reject) => {
        ...
        https.get(oembedUrl, (res) => {
            if (res.statusCode !== 200) {
                console.log(`Failed ${cleanUrl}: ${res.statusCode}`);
                resolve({
                    id: `yt_${videoId}`,
                    title: `Video ${videoId}`,
            ...
            res.on('end', () => {
                try {
                    ...
                    console.log(`Error parsing JSON for ${cleanUrl}`);
                    resolve(null);
                }
            });
        }).on('error', (e) => {
            console.log(`Error network ${cleanUrl}: ${e.message}`);
            resolve(null);
        });
    });
};
